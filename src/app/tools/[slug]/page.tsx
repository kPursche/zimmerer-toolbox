...
export default async function ToolPage({ params }: ToolPageProps) {
  ...
  return (
    <div className="mx-auto w-full max-w-4xl px-4 py-6 sm:px-6 sm:py-8 lg:px-8 lg:py-10">
      ...
      {/* Tool-Inhalt je nach Slug */}
      {slug === "gauenwangen" ? (
        <GauenwangenTool />
      ) : isAvailable ? (
        <Card className="border-dashed bg-s1/60">
          <CardHeader>
            <CardTitle className="text-tx">Tool-Inhalt folgt</CardTitle>
          </CardHeader>
          <CardContent className="text-sm text-mu">
            <p>Implementierung folgt in einem späteren Feature-Ticket.</p>
          </CardContent>
        </Card>
      ) : (
        <Card className="border-dashed bg-s1/60">
          <CardHeader>
            <CardTitle className="text-tx">Dieses Werkzeug ist noch in Vorbereitung</CardTitle>
          </CardHeader>
